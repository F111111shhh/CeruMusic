...
export default {
  ...
  musicSearch(str, page, limit, retryNum = 0) {
    ...
    return searchRequest.promise
      ...
      .catch((error) => {
        if (retryNum >= 8) return Promise.reject(error)
        return wait(250 + retryNum * 150).then(() =>
          this.musicSearch(str, page, limit, retryNum + 1)
        )
      })
  ...
  search(str, page = 1, limit, retryNum = 0) {
    ...
    return this.musicSearch(str, page, limit).then(({ body, meta }) => {
      ...
      const total = Number(meta?.estimate_sum) || list.length

      if (page === 1 && total > 0 && !list.length && retryNum < 3) {
        return wait(300 + retryNum * 200).then(() => this.search(str, page, limit, retryNum + 1))
      }
